...
export class AITerminal {
  ...
  async getSuggestions(input: string): Promise<CommandSuggestion[]> {
    ...
    if (framework === 'Next.js' || framework === 'React') {
      suggestions.push(
        { command: 'npm run build', description: 'Build for production', confidence: 0.8, icon: '📦' },
        { command: 'npm run lint', description: 'Lint code', confidence: 0.7, icon: '✨' }
      );
    }
    ...
    if (framework === 'Vue') {
      suggestions.push(
        { command: 'npm run build', description: 'Build for production', confidence: 0.8, icon: '📦' },
        { command: 'npm run dev', description: 'Start dev server', confidence: 0.9, icon: '🚀' }
      );
    }
